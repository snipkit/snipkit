...
import snipkit, { request, validateEmail } from "@snipkit/next";
...
export async function validate(prev: { message: string }, formData: FormData) {
  ...
  // Access request data that Snipkit needs when you call `protect()` similarly
  // to `await headers()` and `await cookies()` in `next/headers`
  const req = await request();

  const decision = await aj.protect(req, { email });

  // If Snipkit encounters an error, you could fail "open" or you could respond
