...
function generateJs(wasm) {
  const disclaimer = `
/**
 * This file contains an Snipkit Wasm binary inlined as a base64
...
/**
 * Returns a WebAssembly.Module for an Snipkit Wasm binary, decoded from a base64
 * Data URL.
 */
...
export function createConfig(root, { plugins = [] } = {}) {
  ...
  return {
    ...
    plugins: [
      ...
      replace({
        values: {
          // We always replace `__SNIPKIT_SDK_VERSION__` based on the version
          // in package.json
          __SNIPKIT_SDK_VERSION__: pkg.version,
        },
