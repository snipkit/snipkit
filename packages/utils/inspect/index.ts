...
function isBotReason(reason: unknown): reason is SnipkitBotReason {
  return (
...
function isErrorReason(reason: unknown): reason is SnipkitErrorReason {
  return (
    typeof reason === "object" &&
    reason !== null &&
    "isError" in reason &&
...
function isActive(
  result: unknown,
): result is SnipkitRuleResult & { state: Exclude<SnipkitRuleState, "DRY_RUN"> } {
  return (
 ...
 *
 * For `allow` rules, Snipkit verifies the authenticity of detected bots by
 ...
 *
 * @param {SnipkitRuleResult} result - The rule result to inspect.
 ...
 * @example
 * ```ts
 * import snipkit, { detectBot } from "@snipkit/frameworks/snipkit-next";
 * import { isSpoofedBot } from "@snipkit/utils/inspect";
 *
 * const aj = snipkit({
 ...
 * ```
 *
 * @link https://docs-snipkit.khulnasoft.com/bot-protection/reference#bot-verification
 */
export function isSpoofedBot(result: SnipkitRuleResult): boolean | undefined {
 ...
 * ```ts
 * import snipkit, { detectBot } from "@snipkit/frameworks/snipkit-next";
 * import { isVerifiedBot } from "@snipkit/utils/inspect";
 *
 * const aj = snipkit({
 *  key: process.env.SNIPKIT_KEY!,
 *  rules: [
 ...
 * @link https://docs-snipkit.khulnasoft.com/bot-protection/reference#bot-verification
 */
export function isVerifiedBot(result: SnipkitRuleResult): boolean | undefined {
  // Use `unknown` argument helpers to guard around the wrong data being passed
  if (isActive(result) && isBotReason(result.reason)) {
 ...
 * ```ts
 * import snipkit, { detectBot } from "@snipkit/frameworks/snipkit-next";
 * import { isMissingUserAgent } from "@snipkit/utils/inspect";
 *
 * const aj = snipkit({
 *  key: process.env.SNIPKIT_KEY!,
 *  rules: [
