...
describe("isSpoofedBot", () => {
  test("returns true for active bots that are spoofed", () => {
    const activeStates: Exclude<SnipkitRuleState[], "DRY_RUN"> = [
      "RUN",
      "NOT_RUN",
      "CACHED",
    ];
      ...
      expect(
        isMissingUserAgent(
          new SnipkitRuleResult({
            ...
            state,
            conclusion: "DENY",
            reason: new SnipkitErrorReason("missing User-Agent header"),
          }),
        ),
      ).toEqual(true);
      // Local error message
      expect(
        isMissingUserAgent(
          new SnipkitRuleResult({
            ...
            state,
            conclusion: "DENY",
            reason: new SnipkitErrorReason("requires user-agent header"),
          }),
        ),
      ).toEqual(true);
    }
