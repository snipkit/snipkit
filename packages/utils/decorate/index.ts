...
export interface SnipkitCanDecorate {
...
function isHeaderLike(value: SnipkitCanDecorate): value is HeaderLike {
  if (
    "has" in value &&
...
function isResponseLike(value: SnipkitCanDecorate): value is ResponseLike {
...
function extractReason(result: SnipkitRuleResult): SnipkitReason {
  return result.reason;
}

function isRateLimitReason(
  reason: SnipkitReason,
): reason is SnipkitRateLimitReason {
  return reason.isRateLimit();
}

function nearestLimit(
  current: SnipkitRateLimitReason,
  next: SnipkitRateLimitReason,
) {
 ...
 * on an {@link SnipkitDecision} and conforming to the [Rate Limit fields for
 ...
 * @param decision The {@link SnipkitDecision} that was made by calling `protect()` on the SDK.
 */
export function setRateLimitHeaders(
  value: SnipkitCanDecorate,
