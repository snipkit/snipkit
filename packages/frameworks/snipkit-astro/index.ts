...
const resolvedVirtualClientId = "\0SNIPKIT_VIRTUAL_CLIENT";
...
type SnipkitIntegrationOptions<Characteristics extends readonly string[]> = {
...
export default function snipkit<Characteristics extends readonly string[]>(
  {
    rules,
    characteristics,
    client,
    proxies,
  }: SnipkitIntegrationOptions<Characteristics> = { rules: [] },
): AstroIntegration {
  const snipkitImports = new Set();
  const snipkitRules: string[] = [];
  for (const rule of rules) {
    const { importName, code } = integrationRuleToClientRule(rule);
    snipkitImports.add(importName);
    snipkitRules.push(code);
  }
                  ...
                  if (id === resolvedVirtualClientId) {
                    ...
                    return {
                      code: `
                        ...
                        import {
                          ${Array.from(snipkitImports).join(",\n")}
                        } from "snipkit"

                        // Construct an Snipkit client for the virtual module
                        const aj = createSnipkitClient({
                          key: SNIPKIT_KEY,
                          rules: [
                            ${snipkitRules.join(",\n")}
                          ],
      ...
      "astro:config:done": async ({ injectTypes }) => {
        ...
        injectTypes({
          ...
          declare module "snipkit:client" {
            ...
            const client = createSnipkitClient({
              rules: [
                ${snipkitRules.join(",\n")}
              ],
              ${characteristicsInjection}
