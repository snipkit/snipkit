...
export function SnipkitModeToProtocol(mode: SnipkitMode) {
  switch (mode) {
    case "LIVE":
      return Mode.LIVE;
    case "DRY_RUN":
...
export function SnipkitEmailTypeToProtocol(
  emailType: SnipkitEmailType,
): EmailType {
  switch (emailType) {
    case "DISPOSABLE":
...
export function SnipkitEmailTypeFromProtocol(
  emailType: EmailType,
): SnipkitEmailType {
  switch (emailType) {
...
export function SnipkitStackToProtocol(stack: SnipkitStack): SDKStack {
  switch (stack) {
    case "NODEJS":
...
export function SnipkitRuleStateToProtocol(stack: SnipkitRuleState): RuleState {
  switch (stack) {
    case "RUN":
...
export function SnipkitRuleStateFromProtocol(
  ruleState: RuleState,
): SnipkitRuleState {
  switch (ruleState) {
...
export function SnipkitConclusionToProtocol(
...
export function SnipkitConclusionFromProtocol(
...
export function SnipkitReasonFromProtocol(proto?: Reason) {
  if (typeof proto === "undefined") {
    return new SnipkitReason();
  }
  ...
  switch (proto.reason.case) {
    case "rateLimit": {
      ...
      return new SnipkitRateLimitReason({
        max: reason.max,
    ...
    case "edgeRule": {
      return new SnipkitEdgeRuleReason();
    }
    case "shield": {
      const reason = proto.reason.value;
      return new SnipkitShieldReason({
        shieldTriggered: reason.shieldTriggered,
      });
    }
    case "email": {
      const reason = proto.reason.value;
      return new SnipkitEmailReason({
        emailTypes: reason.emailTypes.map(SnipkitEmailTypeFromProtocol),
      });
    }
    case "sensitiveInfo": {
      const reason = proto.reason.value;
      return new SnipkitSensitiveInfoReason({
        allowed: reason.allowed,
        denied: reason.denied,
      });
    }
    case "bot": {
      return new SnipkitErrorReason("bot detection v1 is deprecated");
    }
    case "error": {
      const reason = proto.reason.value;
      return new SnipkitErrorReason(reason.message);
    }
    case undefined: {
      return new SnipkitReason();
    }
...
export function SnipkitReasonToProtocol(reason: SnipkitReason): Reason {
    ...
    return new Reason({
      reason: {
        ...
        value: new EmailReason({
          emailTypes: reason.emailTypes.map(SnipkitEmailTypeToProtocol),
        }),
      },
    });
  }
...
export function SnipkitRuleResultToProtocol(
  ruleResult: SnipkitRuleResult,
): RuleResult {
  return new RuleResult({
    ruleId: ruleResult.ruleId,
    fingerprint: ruleResult.fingerprint,
    ttl: ruleResult.ttl,
    state: SnipkitRuleStateToProtocol(ruleResult.state),
    conclusion: SnipkitConclusionToProtocol(ruleResult.conclusion),
    reason: SnipkitReasonToProtocol(ruleResult.reason),
  });
}

export function SnipkitRuleResultFromProtocol(
  proto: RuleResult,
): SnipkitRuleResult {
  return new SnipkitRuleResult({
    ...
    ttl: proto.ttl,
    state: SnipkitRuleStateFromProtocol(proto.state),
    conclusion: SnipkitConclusionFromProtocol(proto.conclusion),
    reason: SnipkitReasonFromProtocol(proto.reason),
  });
}

export function SnipkitDecisionToProtocol(decision: SnipkitDecision): Decision {
  return new Decision({
    id: decision.id,
    ttl: decision.ttl,
    conclusion: SnipkitConclusionToProtocol(decision.conclusion),
    reason: SnipkitReasonToProtocol(decision.reason),
    ruleResults: decision.results.map(SnipkitRuleResultToProtocol),
  });
}

export function SnipkitIpDetailsFromProtocol(
  ipDetails?: IpDetails,
): SnipkitIpDetails {
  if (!ipDetails) {
    return new SnipkitIpDetails();
  }
...
export function SnipkitDecisionFromProtocol(
  decision?: Decision,
): SnipkitDecision {
  if (typeof decision === "undefined") {
    return new SnipkitErrorDecision({
      reason: new SnipkitErrorReason("Missing Decision"),
      ttl: 0,
      results: [],
  ...
  const results = Array.isArray(decision.ruleResults)
    ? decision.ruleResults.map(SnipkitRuleResultFromProtocol)
    : [];

  switch (decision.conclusion) {
    case Conclusion.ALLOW:
      return new SnipkitAllowDecision({
        id: decision.id,
        ttl: decision.ttl,
        reason: SnipkitReasonFromProtocol(decision.reason),
        results,
        ip: SnipkitIpDetailsFromProtocol(decision.ipDetails),
      });
    case Conclusion.DENY:
      return new SnipkitDenyDecision({
        id: decision.id,
        ttl: decision.ttl,
        reason: SnipkitReasonFromProtocol(decision.reason),
        results,
    ...
    case Conclusion.CHALLENGE:
      return new SnipkitChallengeDecision({
        id: decision.id,
        ttl: decision.ttl,
        reason: SnipkitReasonFromProtocol(decision.reason),
        results,
    ...
    case Conclusion.ERROR:
      return new SnipkitErrorDecision({
        id: decision.id,
        ttl: decision.ttl,
        reason: new SnipkitErrorReason(decision.reason),
        results,
        ip: SnipkitIpDetailsFromProtocol(decision.ipDetails),
      });
    case Conclusion.UNSPECIFIED:
      return new SnipkitErrorDecision({
        id: decision.id,
        ttl: decision.ttl,
        reason: new SnipkitErrorReason("Invalid Conclusion"),
        results,
        ip: SnipkitIpDetailsFromProtocol(decision.ipDetails),
      });
    default: {
      ...
      return new SnipkitErrorDecision({
        ttl: 0,
        reason: new SnipkitErrorReason("Missing Conclusion"),
        results: [],
        ip: SnipkitIpDetailsFromProtocol(decision.ipDetails),
      });
    }
...
export function SnipkitRuleToProtocol<Props extends { [key: string]: unknown }>(
    ...
    return new Rule({
      rule: {
        ...
        value: {
          ...
          mode: SnipkitModeToProtocol(rule.mode),
  ...
  if (isEmailRule(rule)) {
    const allow = Array.isArray(rule.allow)
      ? rule.allow.map(SnipkitEmailTypeToProtocol)
      : [];
    const deny = Array.isArray(rule.deny)
      ? rule.deny.map(SnipkitEmailTypeToProtocol)
      : [];
    return new Rule({
      rule: {
        case: "email",
